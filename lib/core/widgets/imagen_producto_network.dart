...
/// camino (visto en Super Color en Windows — las fotos dejaban de cargar
/// sin mostrar ni error ni "cargando", y quedaban así hasta reiniciar la
/// app entera, que es lo único que limpia el caché de imágenes de Flutter
/// donde había quedado pegado el intento fallido), el pedido se queda
/// esperando para siempre y no hay forma de reintentar sin salir de la
/// pantalla. Acá se baja la foto a mano con un timeout explícito y, si
/// falla, queda un botón para reintentar en el momento.
class ImagenProductoNetwork extends StatefulWidget {
  ...
  const ImagenProductoNetwork({
    ...
    this.loadingColor = const Color(0xFF0D2B4E),
