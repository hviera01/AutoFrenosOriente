...
class CarritoVentaState {
  ...
  // Recargo por tarjeta (%), opcional: a diferencia de la comisión bancaria
  // "de metadata" que se usó en otro de los sistemas hermanos, acá SÍ se
  // suma al total a pagar (así lo cobraba el sistema viejo, con un 5%
  // sugerido) — pero es el cajero quien decide si lo aplica en cada venta
  ...
  // ISV real del 15%, igual que Super Color: este negocio sí lo cobra de
  // verdad en cada venta (a diferencia del taller de barbería hermano, donde
  // se confirmó con datos reales que no aplicaba), así que se calcula
  // siempre, sin condicionarlo al tipo de documento elegido.
  ...
  /// Monto real del recargo por tarjeta (0 si no se aplicó ninguno).
  double get montoRecargoTarjeta => metodoPago == 'Tarjeta' ? redondearMoneda(_totalConImpuestoBase * (porcentajeTarjeta / 100)) : 0;

  /// Redondeo a lempira entero: si el residuo es >= .90 sube, si no baja.
  double get totalAPagar {
    final t = _totalConImpuestoBase + montoRecargoTarjeta;
...
class CarritoVentaNotifier extends Notifier<CarritoVentaState> {
  ...
  /// de búsqueda), con cantidad 1 y sin descuento por defecto. [precioSeleccionado],
  /// si viene, ya es el precio CON ISV tal como se muestra en el buscador
  /// (igual que en Super Color); acá se le quita el impuesto antes de
  /// guardarlo, para que precioVenta siempre quede sin ISV en el carrito.
  void agregarProductoDirecto(ProductoModel producto, {double? precioSeleccionado, double precioCompraUsado = 0, bool reembasado = false}) {
    ...
    // Sin redondear acá: precioVenta (sin ISV) no siempre es un número
    // "limpio" de 2 decimales -por ejemplo 100/1.15- y redondearlo antes de
    // multiplicarlo de nuevo por 1.15 para mostrar o sumar el total es lo
    // que causaba precios como 100.01 en vez de 100.00 (dos redondeos en
    // cadena). Se redondea una sola vez, recién al mostrar/calcular un total.
  ...
  /// descuento de línea directamente desde la tabla, recalculando el
  /// subtotal. Usa copyWith (no reconstruye el item desde cero) para no
  /// perder esServicio si el cajero toca el precio o la cantidad después.
  ...
  void establecerVehiculo(String v) => state = state.copyWith(vehiculo: v);
