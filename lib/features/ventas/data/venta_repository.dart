...
class VentaRepository {
  ...
  Future<VentaModel> registrarVenta({
    ...
    // % de recargo por tarjeta si metodoPago == 'Tarjeta' (ver
    // carrito_provider): ya está incluido en totalAPagar, se guarda aparte
    // solo para que el detalle de la venta pueda mostrarlo desglosado.
    double porcentajeTarjeta = 0,
    // Vehículo asociado a la venta (texto libre, opcionalmente autocompletado
    // desde el registro de Vehículos del cliente). Igual que en el sistema
    // viejo: es una foto de texto en el momento, no una referencia viva.
    String vehiculo = '',
  }) async {
    ...
    await _db.runTransaction((transaction) async {
      ...
      // que todavía no son una venta concretada. El ISV siempre se cobra en
      // este negocio, así que siempre se suma acá.
      if (tipoDocumento != 'Cotizacion') {
        for (final item in items) {
          final ref = _db.collection('productos').doc(item.idProducto);
          final precioBase = item.precioVenta * (1 - item.descuentoPorcentaje / 100);
          final precioFinal = redondearMoneda(precioBase * 1.15);
